...
public enum StatMapping {
    ...
    private static final BiMap<ResourceLocation, BukkitStatistic> statistics;

    static {
        ImmutableBiMap.Builder<ResourceLocation, BukkitStatistic> statisticBuilder = ImmutableBiMap.builder();
    ...
    StatMapping(ResourceLocation minecraftKey) {
    ...
    public static Stat getMaterialStatistic(BukkitStatistic stat, Item item) {
        try {
            if (stat == BukkitStatistic.MINE_BLOCK) {
                return Stats.BLOCK_MINED.get(Block.byItem(item));
            }
            if (stat == BukkitStatistic.CRAFT_ITEM) {
                return Stats.ITEM_CRAFTED.get(item);
            }
            if (stat == BukkitStatistic.USE_ITEM) {
                return Stats.ITEM_USED.get(item);
            }
            if (stat == BukkitStatistic.BREAK_ITEM) {
                return Stats.ITEM_BROKEN.get(item);
            }
            if (stat == BukkitStatistic.PICKUP) {
                return Stats.ITEM_PICKED_UP.get(item);
            }
            if (stat == BukkitStatistic.DROP) {
                return Stats.ITEM_DROPPED.get(item);
            }
    ...
    public static Stat getEntityStatistic(BukkitStatistic stat, EntityType entityType) {
        ...
        if (stat == BukkitStatistic.ENTITY_KILLED_BY) {
            return Stats.ENTITY_KILLED_BY.get(entityType);
        }
        return null;
    }
}
