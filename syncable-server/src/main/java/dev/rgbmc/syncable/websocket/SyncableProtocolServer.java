...
public class SyncableProtocolServer extends WebSocketServer {
    ...
    public void onMessage(WebSocket conn, String message) {
        ...
        CommandHandler handler = CommandHandlers.getHandler(jsonObject.get("commandId").getAsInt(), jsonObject);
        if (handler.callable()) {
    ...
    public void onError(WebSocket conn, Exception ex) {
        SyncableServer.getInstance().getLogger().warning("Error encountered when handling connection: " + conn.getRemoteSocketAddress().getAddress().getHostAddress());
        ex.printStackTrace();
    }
