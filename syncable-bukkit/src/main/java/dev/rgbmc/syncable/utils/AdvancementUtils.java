...
public class AdvancementUtils {
    public static List<AdvancementData> getAdvancements(Player player) {
        ...
        advancementIterator.forEachRemaining(advancement -> {
            Map<String, Date> awardedCriteria = new HashMap<>();
            AdvancementProgress progress = player.getAdvancementProgress(advancement);
            progress.getAwardedCriteria().forEach(key -> {
    ...
    public static void setAdvancements(Player player, List<AdvancementData> advancements) {
        ...
        final Iterator<Advancement> advancementIterator = Bukkit.advancementIterator();
        advancementIterator.forEachRemaining(advancement -> {
            advancements.stream().filter(data -> data.getNamespace().equalsIgnoreCase(advancement.getKey().toString())).findFirst().ifPresentOrElse(advancementData -> {
                ...
                AdvancementProgress progress = player.getAdvancementProgress(advancement);
                progress.getAwardedCriteria().forEach(criteria -> player.getAdvancementProgress(advancement).revokeCriteria(criteria));
            });
        });
