    ...
    public void deserialize(UUID playerId, String data) {
        ...
        for (String str : jsonObject.keySet()) {
            ...
            switch (statistic.getType()) {
                case ENTITY -> player.setStatistic(statistic, EntityType.valueOf(param[1]), jsonObject.get(str).getAsInt());
                case BLOCK, ITEM -> player.setStatistic(statistic, Material.valueOf(param[1]), jsonObject.get(param[1]).getAsInt());
            ...
            switch (statistic.getType()) {
                case ENTITY -> {
                    for (EntityType entityType : EntityType.values()) {
                        jsonObject.addProperty(statistic.name() + ":" + entityType.name(), player.getStatistic(statistic, entityType));
                    }
                }
                case BLOCK, ITEM -> {
                    for (Material material : Material.values()) {
                        jsonObject.addProperty(statistic.name() + ":" + material.name(), player.getStatistic(statistic, material));
                    }
                }
                default -> {
